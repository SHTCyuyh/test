...
from NLOS_video.models.vit_utils import DropPath, to_3tuple, trunc_normal_
...
class PatchEmbed(nn.Module):
    ...
    """
    def __init__(self, img_size=128, patch_size=64, in_chans=1, embed_dim=768):
        super().__init__()
        ...
        self.proj = nn.Conv3d(in_chans, embed_dim, kernel_size=16, stride=16)

    def forward(self, x):
        B, C, T, D, H, W = x.shape
        x = rearrange(x, 'b c t d h w -> (b t) c d h w')
        x = self.proj(x)
        W = x.size(-1)
        x = x.flatten(2).transpose(1, 2)  #(b*c) 768 (8*8*8)
        return x, T, W
...
class VisionTransformer(nn.Module):
    """ Vision Transformere
    """
    def __init__(self, img_size=128, patch_size=16, in_chans=1, num_classes=3, embed_dim=768, depth=12,
    ...
    def forward_features(self, x):
        B = x.shape[0]
        x, T, W = self.patch_embed(x)  #x 16 512(d*h*w) 768
        cls_tokens = self.cls_token.expand(x.size(0), -1, -1)
        x = torch.cat((cls_tokens, x), dim=1)  #(bc) (dhw+1) (hidden)
...
class TimeSformer(nn.Module):
    def __init__(self, img_size=128, patch_size=16, num_classes=24, num_frames=8, attention_type='divided_space_time',  pretrained_model='', **kwargs):
        ...
        self.num_patches = (img_size // patch_size) * (img_size // patch_size) *(img_size // patch_size)
        # if self.pretrained:
...
if __name__ == '__main__':
    model = TimeSformer()
    x =np.zeros((2, 1, 8, 128, 128, 128), dtype=np.float32)   # b c t d h w
