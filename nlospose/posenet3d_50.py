...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.head = DeconvHead(2048, 3, 256, 4, 1, 24, 64, True)
    ...
    def forward(self, x):
        x = self.conv1(x)  #2 1 256 256 256 -> 2 64 128 128 128
        x = self.bn1(x)     # 2 64 128 128 128
        x = self.relu(x)      #2 64 128 128 128
        if not self.no_max_pool:
            x = self.maxpool(x)        #2 64 64 64 64 

        x = self.layer1(x)  # 
        x = self.layer2(x)  # [2, 256, 64, 64, 64]) -> torch.Size([2, 512, 32, 32, 32]) 
        x = self.layer3(x) #torch.Size([2, 512, 32, 32, 32]) ->torch.Size([2, 1024, 16, 16, 16])
        x = self.layer4(x)   #torch.Size([2, 1024, 16, 16, 16]) ->torch.Size([2, 2048, 8, 8, 8])

        # x = self.avgpool(x)

        # x = x.view(x.size(0), -1)
        # x = self.fc(x)
        x = self.head(x)  # torch.Size([2, 2048, 8, 8, 8])  -> torch.Size([2, 24, 64, 64, 64])
...
def get_config():
    ...
    _C.model_depth = 50
    _C.n_classes = 700
    _C.n_input_channels = 1
    _C.shortcut_type = 'B'
    _C.conv1_t_size = 7 
    _C.conv1_t_stride = 2 
    _C.no_max_pool = False
    _C.widen_factor = 1.0 
    return _C
...
def get_pose_net():
    cfg = get_config()
    model = generate_model(model_depth=cfg.model_depth,
...
if __name__ == '__main__':
    ...
    input = torch.ones((2,1,256,256,256)).to("cuda:1")
    output = model(input)
    print(output.shape)
    print('finish')