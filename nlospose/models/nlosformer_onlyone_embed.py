...
class Attention3D2(nn.Module):
    ...
    def forward(self, x):
        ...
        # attn_mask = attn_mask.to(device=cfg.DEVICE)
...
class Attention3D_fusion(nn.Module):
    ...
    def forward(self, x, x2, mask=None):
        ...
        attn = q2 @ k.transpose(-2, -1) * self.scale   #B NUM_heads dd*dw*dh dd*dw*dh
        if self.use_mask:
            nW = mask.shape[0]
            attn = attn.view(B // nW, nW, self.num_heads, N, N) + mask.unsqueeze(1).unsqueeze(0)
            attn = attn.view(-1, self.num_heads, N, N)
...
class nlosformer_singlefusion(nn.Module):
    def __init__(self, num_joints=24, input_size=(64,64,64),patch_size=(4,4,4), in_chans=1, embed_dim=96,
        ...
        self.linear = nn.Linear(self.num_patchs *embed_dim , out_dim)
