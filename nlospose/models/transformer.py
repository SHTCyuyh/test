...
class Attention(nn.Module):
    ...
    def forward(self, x, einops_from, einops_to, mask = None, cls_mask = None, rot_emb = None, **einops_dims):
        ...
        q_, k_, v_ = map(lambda t: rearrange(t, f'{einops_from} -> {einops_to}', **einops_dims), (q_, k_, v_)) # ‘b (n f) d -> (b n) f d’
...
if __name__ == '__main__':
    ...
    video = torch.randn(1, 20, 3, 224, 224) # (dim x frames x channels x image_size x image_size)

    # PlotVolume(rearrange(video, 'a b '))
