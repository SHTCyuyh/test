...
class UNet3d(nn.Module):
    def __init__(self, in_channels, n_channels):
        ...
        self.out = Out(n_channels, in_channels)
    ...
    def forward(self, x):
        ...
        out = self.dec1(x5, x4)
        out = self.dec2(out, x3)
        out = self.dec3(out, x2)
...
if __name__ == '__main__':
    ...
    input = input.to('cuda')
    model = UNet3d(in_channels=1, n_channels=4).to('cuda')
    # input = torch.ones((2,4,128,240,240)).to('cuda')
    output = model(input)
    # summary(model,(4,128,256,256))
    print('finish')
