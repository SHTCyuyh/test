...
class SwinTransformer3D(nn.Module):
    ...
    def __init__(self,
        ...
        self.head = nn.Sequential(
            nn.LayerNorm(self.num_features * 16 * 16 * 16),
            nn.Linear(self.num_features * 16 * 16 * 16, self.joints_num * self.out_dim),
        )
    ...
    def forward(self, x):
        ...
        x = self.pos_drop(x)  #b c dd dh dw
        ...
        # pred = self.heatmap(x)
        pred = rearrange(pred, 'b (n c) -> b n c', n = self.joints_num)
...
class heatmap(nn.Module):
    ...
    def forward(self, preds, num_joints =24):
        # gt_joints = args[0]
        # gt_joints_vis = args[1]  #torch.Size([1, 72])
...
def generate_3d_integral_preds_tensor(heatmaps, num_joints, x_dim, y_dim, z_dim):
    ...
    accu_x = accu_x * torch.arange(x_dim).float().cuda('cuda:3')[None,:]
    accu_y = accu_y * torch.arange(y_dim).float().cuda('cuda:3')[None,:]
    accu_z = accu_z * torch.arange(z_dim).float().cuda('cuda:3')[None,:]
...
if __name__ == '__main__':
    ...
    x = x.to(device='cuda:3')
    model = model.to(device='cuda:3')
    out = model(x)  #[B ,c d h w]
    print(out.shape)
