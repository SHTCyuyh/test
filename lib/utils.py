...
def crop_video(video, framenum, res):
    ...
    sumdata = torch.zeros((len(video),), dtype=torch.float32)
    for i, frame in enumerate(video):
        sumdata[i] = torch.sum(frame)
    
    idxs = []
    vals = []
    for i in range((len(video) - framenum) // res):
        idxs.append([i * res, i * res + framenum])
        vals.append(torch.sum(sumdata[i * res:i * res + framenum]))
    # find max
...
def crop(input):
    ...
    crop_input = torch.zeros((input.shape[0], input.shape[1], 128, input.shape[3], input.shape[4]), dtype=torch.float32)
    for batch in range(input.shape[0]):
        crop_input[batch, 0], _, __ = crop_video(input[batch, 0], 128, 2)

    return crop_input


def downsample(meas, k):
    for i in range(k):
